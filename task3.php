...
<body>
  ...
  <form method="get">
    ...
    <select name="rank" id="rank">
      ...
      <option value="Рядовий">Рядовий</option>
    </select>
    <br>
  ...
  if (isset($_GET['submit'])) {
    ...
    if (empty($rank)) {
      $rank_condition = "s.soldier_rank IN ('Старшина', 'Сержант', 'Прапорщик', 'Ефрейтор', 'Рядовий')";
    } else {
    ...
    if ($type == 'army') {
      ...
      $sql = "SELECT s.soldier_id, s.name, s.soldier_rank, s.department_id 
            FROM soldier s 
            INNER JOIN department ON s.department_id = department.department_id
            INNER JOIN vzvod ON department.vzvod_id = vzvod.vzvod_id AND vzvod.name = '$name'
            WHERE $rank_condition";
    } 
    // Виконання запиту
